...
(define inc-count-list!
	(lambda (ls env) 
		(fold-left (lambda (acc head)
			(let ([head-val (apply-env env head)]) 
				(if (ref-val? head-val)
					(inc-count! head-val))) (cons head acc)) '() ls)))

(define dec-count-list!
	(lambda (ls env) 
		(fold-left (lambda (acc head)
			(let ([head-val (apply-env env head)]) 
				(if (ref-val? head-val)
					(dec-count! head-val))) (cons head acc)) '() ls)))

;;increments the reference count of for the number of times an element in the store is refrenced
(define inc-count!
  (lambda (ev)
    (let ([ref (expval->ref ev)])
      (let ([val (vector-ref the-store! ref)])
    (vector-set! the-store! ref (cons (car val) (list (+ 1 (cadr val)))))))))

;;decrements the reference count of for the number of times an element in the store is refrenced
(define dec-count!
  (lambda (ev)
    (let ([ref (expval->ref ev)])
      (let ([val (vector-ref the-store! ref)])
    (vector-set! the-store! ref (cons (car val) (list (- (cadr val) 1))))) (garbage-collector!))))
...
(define value-of
  (lambda (prog env)
    (cases program prog
	   ...
	   [def-prog (var exp)
	     (let ([ref (newref! (value-of-exp exp env))])
					(inc-count! ref)
	       (cons (unit-val) (extend-env var ref env)))]
  ...
  (lambda (exp env)
    (cases expression exp
      ...
      [let-exp (var exp1 exp2)
		    (let ([ref (newref! (value-of-exp exp1 env))])
		      (inc-count! ref)
		      (let ([val (value-of-exp exp2 (extend-env var ref env))])
			(dec-count! ref)
			val))]
		   ...
		   [(= (length params) (length rands))
			...
			([pairs (map list params rands)]
			 ...
			 [new-env (fold-left extend-with-pair saved-env pairs)])
					(inc-count-list! params new-env)
					(let ([val (value-of-exp body new-env)])
		      	(dec-count-list! params new-env) (garbage-collector) val))]
      ...
      [setref-exp (exp1 exp2) (setref! (value-of-exp exp1 env) (value-of-exp exp2 env)) (the-undertaker) (unit-val)] 
